...
export default function ManifestQrCard({ manifest, orderNumber, itemCount }: { manifest: string; orderNumber: string; itemCount: number }) {
  ...
  return (
    <div className="rounded-2xl border-2 border-dashed border-yellow-400 bg-zinc-900 p-6 text-center text-white">
      <div className="text-sm text-zinc-400">BORDEREAU PALETTE — 1 seul code pour tous les articles</div>
      ...
      <button onClick={() => window.print()} className="mt-3 rounded-lg bg-yellow-400 text-black font-bold px-4 py-2 print:hidden">🖨️ Imprimer le bordereau</button>
    </div>
  );
}
