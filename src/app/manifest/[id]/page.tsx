    ...
    return (
      <div className="bg-mesh flex min-h-screen items-center justify-center px-4">
        <div className="glass max-w-md rounded-[28px] p-8 text-center">
          ...
          <h1 className="mt-3 text-2xl font-black tracking-tight">Manifest not found</h1>
          <p className="mt-2 text-sm text-zinc-400">This transfer reference is invalid or no longer available.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="relative mx-auto max-w-3xl">
        <div className="premium-card rounded-[28px] p-5 sm:p-7">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-zinc-500">Transfer manifest</p>
              <h1 className="mt-2 text-3xl font-black tracking-[-0.05em]">🚚 Site B dispatch</h1>
            </div>
            <div className="inline-flex items-center rounded-full border border-ice/30 bg-ice/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-ice-soft">
              {(t as any).work_orders?.order_number ?? "Pending order"}
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-zinc-300">
              <span className="font-bold text-white">Status:</span> <span className="text-fire-soft">{t.status}</span>
              {t.verified_at ? ` · verified ${t.verified_at}` : " · awaiting Site B scan"}
            </div>
          </div>
        </div>
      </div>
