    ...
    <div className="stagger space-y-5">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="glass flex items-center gap-3 p-4">
          ...
          <div><p className="text-2xl font-black">{stocks.length}</p><p className="text-[11px] font-bold uppercase tracking-widest text-zinc-500">Stock items</p></div>
        </div>
        ...
        <div className="glass flex items-center gap-3 p-4">
          <span className={cn("grid h-11 w-11 place-items-center rounded-xl", lowCount > 0 ? "bg-red-400/15 text-red-300" : "bg-emerald-400/15 text-emerald-300")}>
            {lowCount > 0 ? <AlertTriangle size={20} /> : <CheckCircle size={20} />}
          </span>
          <div><p className="text-2xl font-black">{lowCount}</p><p className="text-[11px] font-bold uppercase tracking-widest text-zinc-500">Low stock alerts</p></div>
        </div>
      </div>

      {pendingSemi > 0 && (
        <GlassCard className="border-amber-400/30 bg-gradient-to-r from-amber-400/10 to-transparent">
      ...
      <div className="glass flex gap-1.5 p-2">
        {(["inventory", "movements", "semi"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={cn("rounded-xl border px-4 py-2 text-sm font-bold transition",
            tab === t ? "border-fire/40 bg-fire/15 text-white" : "border-white/10 text-zinc-400 hover:text-white")}>
          ...
          {stocks.length === 0 ? <Empty icon="📦" title="No stock items" hint="Add your first material above." /> : (
            <div className="space-y-1.5">
              {stocks.map((s) => {
                ...
                return (
                  <div key={s.id} className="glass-soft flex items-center gap-3 p-3">
                    <span className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-lg text-lg",
                      health === "ok" ? "bg-emerald-400/15" : health === "low" ? "bg-amber-400/15" : "bg-red-400/15")}>
                      {health === "ok" ? "✅" : health === "low" ? "⚠️" : "🔴"}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-bold">{s.name}</p>
                      <p className="text-xs text-zinc-500">{fmtQty(s.quantity, s.unit)} · Reserved {fmtQty(s.reserved, s.unit)} · Available {fmtQty(s.available, s.unit)}</p>
                    </div>
                    <div className="flex shrink-0 gap-1.5">
                      ...
                      <button onClick={async () => { if (confirm(`Delete "${s.name}"?`)) await deleteStockItem(s.id); }}
                        className="grid h-8 w-8 place-items-center rounded-xl border border-red-400/25 bg-red-400/10 text-red-300" title="Delete"><Trash2 size={14} /></button>
                    </div>
                  </div>
                );
              })}
        ...
        <GlassCard>
          ...
          {movements.length === 0 ? <Empty icon="🔄" title="No movements yet" hint="Activity will appear here once production starts." /> : (
            <div className="space-y-1.5 max-h-[500px] overflow-y-auto pr-1">
              {movements.map((m) => (
                <div key={m.id} className="glass-soft flex items-center gap-3 p-3 text-sm">
                  <span className={cn("rounded-lg px-2 py-0.5 text-xs font-black",
                    m.movement_type === "consume" ? "bg-red-400/15 text-red-300" :
                    m.movement_type === "reserve" ? "bg-amber-400/15 text-amber-300" :
                    m.movement_type === "adjust" ? "bg-ice/15 text-ice-soft" : "bg-emerald-400/15 text-emerald-300")}>
                  ...
                  <span className="hidden text-xs text-zinc-500 sm:block">{new Date(m.created_at).toLocaleString("fr-FR")}</span>
                </div>
              ))}
            </div>
          )}
              ...
              {semiStock.map((s) => (
                <div key={s.id} className="glass-soft flex items-center gap-3 p-3.5">
                  <span className={cn("grid h-10 w-10 shrink-0 place-items-center rounded-xl text-lg",
                    s.status === "RELEASED" ? "bg-emerald-400/15" : "bg-amber-400/15")}>
                    {s.status === "RELEASED" ? "✅" : "🏗️"}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-bold">{s.work_order_items?.product_name}</p>
                    <p className="text-xs text-zinc-500">×{s.quantity} · {new Date(s.created_at).toLocaleDateString("fr-FR")}</p>
                  </div>
...
function AddItemModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <form className="glass w-full max-w-md space-y-3 p-6" onSubmit={async (e) => {
      ...
      <input required placeholder="Material name" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="glass-input w-full px-4 py-2.5 text-sm" />
      <div className="grid grid-cols-3 gap-2">
        <select value={f.unit} onChange={(e) => setF({ ...f, unit: e.target.value })} className="glass-input px-2 py-2.5 text-sm">
          {["pcs", "m", "m²", "kg", "L", "pcs"].map((u) => <option key={u} value={u} className="bg-zinc-900">{u}</option>)}
        </select>
        <input type="number" step="any" min={0} value={f.quantity} onChange={(e) => setF({ ...f, quantity: Number(e.target.value) })}
          className="glass-input px-2 py-2.5 text-sm" placeholder="Qty" />
        <input type="number" step="any" min={0} value={f.alert_threshold} onChange={(e) => setF({ ...f, alert_threshold: Number(e.target.value) })}
          className="glass-input px-2 py-2.5 text-sm" placeholder="Alert at" />
      </div>
...
function AdjustModal({ item, onClose }: { item: StockRow; onClose: () => void }) {
  ...
  return (
    <form className="glass w-full max-w-md space-y-3 p-6" onSubmit={async (e) => {
      ...
      <p className="text-sm text-zinc-400">Current: {fmtQty(item.quantity, item.unit)}</p>
      <input type="number" step="any" min={0} value={qty} onChange={(e) => setQty(Number(e.target.value))} className="glass-input w-full px-4 py-3 text-lg font-bold" />
      <input placeholder="Adjustment reason (optional)" value={note} onChange={(e) => setNote(e.target.value)} className="glass-input w-full px-4 py-2.5 text-sm" />
